...
> **Two staging verifications still owed (do before fully trusting in prod):** (1) #123 — run one real **Full History Backfill** end-to-end (instant disable, honest running→success, no "Failed to fetch", mid-run second click returns 409); (2) #114 — confirm a StackAdapt-only campaign (e.g. 26022 CATIE) shows real reach/frequency + diagnostics score on dedup reach.
