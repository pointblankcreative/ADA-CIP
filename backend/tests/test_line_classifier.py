...
# ── Search channel → conversion (AI-043) ─────────────────────────────


class TestSearchChannel:
    def test_search_rsa_bare_objective_is_conversion(self):
        """A Google Search RSA with only "Search" as its objective — no
        conversion keyword — grades as CONVERSION on channel_category alone.
        This is the core ticket fixture: it must NOT land on the persuasion
        pillar (the D5 'delivering in bursts' signal is meaningless for Search).
        """
        line = _line("Search", channel_category="Search", platform_id="google_ads")
        assert classify_line(line) == CampaignType.CONVERSION
        assert is_conversion(line)
        assert not is_persuasion(line)

    def test_search_channel_no_objective_is_conversion(self):
        line = _line(None, channel_category="Search", platform_id="google_ads")
        assert classify_line(line) == CampaignType.CONVERSION

    def test_search_channel_case_insensitive(self):
        line = _line("Search", channel_category="search")
        assert classify_line(line) == CampaignType.CONVERSION

    def test_search_channel_overrides_awareness_objective(self):
        """channel_category beats the objective keyword: an awareness-shaped
        objective on a Search line is still conversion."""
        line = _line("Awareness (Video Views)", channel_category="Search")
        assert classify_line(line) == CampaignType.CONVERSION

    def test_non_search_channels_unchanged(self):
        """Display/Video/Social/Audio awareness lines keep PERSUASION."""
        for cat in ("Display", "Video", "Social", "Audio", "Digital"):
            line = _line("Awareness (Video Views)", channel_category=cat)
            assert classify_line(line) == CampaignType.PERSUASION, cat

    def test_search_string_maps_to_conversion(self):
        """Daily/adset path (no channel_category): the word 'search' in the
        objective/name marks a conversion channel."""
        assert classify_objective_string("Search - RSA") == CampaignType.CONVERSION
        assert (
            classify_objective_string(None, "26031 Google Search RSA")
            == CampaignType.CONVERSION
        )

    def test_research_does_not_false_trigger(self):
        """Regression guard: \\bsearch\\b must not catch 'research'."""
        assert (
            classify_objective_string("Market Research Awareness")
            == CampaignType.PERSUASION
        )
        assert classify_campaign_name("25042 Research Panel Awareness") == CampaignType.PERSUASION
...
class TestPartitionLines:
    ...
    def test_search_line_partitions_to_conversion(self):
        """A Search-channel line lands in CONVERSION while channel_category=None
        awareness lines stay in PERSUASION."""
        lines = [
            _line("Awareness (Video Views)", "aware"),
            _line("Search", "search", channel_category="Search", platform_id="google_ads"),
        ]
        out = partition_lines(lines)
        persuasion_ids = {l.line_id for l in out[CampaignType.PERSUASION]}
        conversion_ids = {l.line_id for l in out[CampaignType.CONVERSION]}
        assert persuasion_ids == {"aware"}
        assert conversion_ids == {"search"}
