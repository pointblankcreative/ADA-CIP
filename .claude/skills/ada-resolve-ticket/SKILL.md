...
`SKILL_DIR` below is the directory containing this file. Run every script as `"$SKILL_DIR/scripts/<name>"`. Preconditions: `ASANA_PAT` is set in the environment; the repo `.venv` exists; for auto-promote, either `gh` is authenticated or you have push access to `main`. **If those preconditions don't hold, you're running under Claude Code on the web — read "Operating notes" at the end first; the pipeline still applies but you drive Asana over MCP and, instead of `promote.sh`, you deliver by **merging the PR to `main` yourself** for an `auto` ticket (the web equivalent of `promote.sh`) or handing off a **draft PR** for a `park` ticket.**
...
- **Deliver by merging the PR (`auto`) or handing off a draft PR (`park`).** The web session is pinned to one designated branch and can't run `promote.sh`, but it CAN reach `main` through a PR. Commit to the designated branch and push (fix the commit identity if the stop-hook flags it: `git config user.email noreply@anthropic.com && git commit --amend --reset-author`). Then split on `promote_decision`:
  - **`auto` → merge it to `main` yourself.** Open the PR against `main` as a **ready (non-draft) PR**, then merge it with the GitHub MCP (`merge_pull_request` with `merge_method: "merge"` to match the repo's merge-commit history). Merging to `main` IS the staging deploy (`deploy.yml` fires on push-to-`main`, ~7 min) — this is the web equivalent of `promote.sh`, and it goes no further than staging (production is a separate manual push to the `production` branch, **never** done here). The autonomy boundary still holds unchanged: only `auto` tickets ever reach this branch — anything `claim.py` or the reviewers flagged `park` (BQ/ingestion/schema/`.sql`, or a "review before staging" body) is **never** merged. After a clean merge, post the **staged** Asana handoff (mirror `handoff.py staged`): move the ticket to the **Ready In Staging** section (`1215988107013673`), set Status = Completed (`{"1215988107013691":"1215988107013694"}`), Stage = Launch (`{"1215988107013696":"1215988107013699"}`), and Ready For → Frazer (`{"1216308984626884":"1216308984626885"}`) for the prod promote — with a comment summarising the fix and linking the merged PR.
    - **Smoke on web is best-effort, post-merge.** You can't cookie-curl staging synchronously (deploy takes ~7 min), so instead watch the deploy: `subscribe_pr_activity` on the PR and/or schedule a ~10-min self check-in to confirm the `Deploy CIP` run on `main` goes green. If the deploy fails, comment the failure on the ticket and leave Ready For → Frazer — **do NOT revert `main`**.
    - **If the merge is refused** (branch protection, insufficient token permission, or `mergeable_state` ≠ `clean`): fall back to the `park`-style hand-off below — leave the PR open, post the Asana handoff (Ready For → Frazer) explaining the branch is pushed and needs his manual merge, and note the refusal reason. This is the old exit-`5` behaviour.
  - **`park` → hand back a draft PR.** Commit to the designated branch, push, open a **draft PR**, and post the Asana park/hand-off (Ready For → Frazer, Status stays In progress) explaining the open question or the must-park reason. Never merge a `park` ticket.
  - **The designated branch is single-use.** Once its PR merges, restart it from latest `main` before the next change (`git fetch origin main && git checkout -B <branch> origin/main`); a PR opened off the refreshed branch is a NEW PR, never the merged one. Never stack new commits on already-merged history.
